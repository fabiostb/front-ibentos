...
export class IbentoService {
  ...
  private readonly API_URL = 'http://localhost:8080/api/ibento';
  ...
  getIbentos(): Observable<IbentoList[]> {
    return this.http.get<IbentoList[]>(this.API_URL)
      .pipe(
  ...
  searchIbentos(term: string): Observable<Ibento[]> {
    ...
    return this.http.get<Ibento[]>(`${this.API_URL}/find?name=${term}`).pipe(
      tap(x => x.length ?
  ...
  addIbento(ibento: Ibento): Observable<Ibento> {
    return this.http.post<Ibento>(this.API_URL, ibento, this.httpOptions).pipe(
  ...
  updateIbento(ibento: Ibento): Observable<Ibento> {
    return this.http.put<Ibento>(this.API_URL, ibento, this.httpOptions).pipe(
